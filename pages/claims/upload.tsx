...
const useStyles = makeStyles( ( { palette, shape, spacing }: Theme ) => createStyles( {
	'@keyframes progress': {
		'0%': {
...
const ClaimUploadPage: React.FC<ClaimUploadPageProps> = () => {
	const [ uploadClaims, { data, loading, error } ] = useUploadClaimsMutation();
	const {
		getRootProps,
		getInputProps,
		isDragAccept,
		isDragReject,
	} = useDropzone( { accept: 'text/csv', onDropAccepted: ( files ) => uploadClaims( { variables: { file: files[ 0 ] } } ) } );
	...
	return (
		<Container maxWidth="md">
			...
			{
				hasUploadError && <Alert severity="error" className={ classes.uploadError }>There was an error processing your uploads.</Alert>
			}
			{
				! loading && ( ! data || hasUploadError ) && (
					<Paper { ...getRootProps() } className={ dropzoneClasses } elevation={ isDragAccept ? 3 : 0 }>
						<input { ...getInputProps() } />
				...
				data?.uploadClaims.success && (
					<>
						<Paper className={ classes.uploadSuccess }>
							<Typography variant="h6" component="p" align="center">
								{ data.uploadClaims.claimsProcessed } claims processed!
							</Typography>
						</Paper>
						<Box my={ 4 }>
...
export async function getStaticProps(): Promise<{ props: ClaimUploadPageProps }> {
	return {
		props: {
			title: 'Upload Claims',
		},
