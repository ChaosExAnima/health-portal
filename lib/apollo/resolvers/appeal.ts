...
import type { Appeal, Call, Claim, Note, Provider } from 'lib/db/entities';

const getAppeals: QueryResolver< 'getAppeals' > = async (
	parent,
	{ offset = 0, limit = 100 },
	{ dataSources: { db } }
) => {
	const [ appeals, totalCount ] = await db.findAndCount< Appeal >(
		'Appeal',
		offset,
		limit
	);
...
const appeal: QueryResolver< 'appeal' > = async (
	parent,
	{ slug },
	{ dataSources: { db } }
) => {
